...
parser = argparse.ArgumentParser(description="Correlate OpenThread telemetry with PPK power data for Risk logs")
...
telemetry_files = [f for f in log_dir.glob('risk_telemetry_*.csv')]
...
log_txt_path = latest_telemetry.parent / f"risk_logs_{latest_telemetry.stem.split('_')[-1]}.txt"
try:
    with open(log_txt_path, 'r', encoding='utf-8') as f:
        first_line = f.readline().strip()
        # Parse time from "[HH:mm:ss.fff] Starting ChildTelemetry_Risk.ps1"
        match = re.match(r'\[(\d{2}:\d{2}:\d{2}\.\d{3})\] Starting ChildTelemetry_Risk.ps1', first_line)
        if match:
            time_str = match.group(1)
            # Combine with date from filename
            date_str = latest_telemetry.stem.split('_')[-1][:8]  # yyyyMMdd
...
output_path = log_dir / f"correlated_risk_telemetry_{datetime.now(tz_cest).strftime('%Y%m%d_%H%M%S')}.csv"
