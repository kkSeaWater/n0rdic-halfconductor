...
Robust serial logger for Nordic nRF52 OpenThread CLI (ot-cli).

- Continuously reads all serial lines (no data loss).
- Logs raw lines with timestamps to nordic_log_*.txt
- Extracts:
...
Usage example:
    ...
    python pyserial_nordic.py --port COM8 --baud 115200 --duration 300 --interval 1.0
"""
...
def run_logger(port: str,
    ...
    log_txt = f"nordic_log_{ts_base}.txt"
    telemetry_csv = f"nordic_telemetry_{ts_base}.csv"
    parent_events_csv = f"nordic_parent_events_{ts_base}.csv"
    ...
    # write headers
    raw_log_file.write(f"[INFO] Nordic logger started at {iso_now()}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Nordic nRF52 OpenThread serial logger")
